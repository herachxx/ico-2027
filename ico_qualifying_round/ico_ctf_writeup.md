...
Пишу этот райтап так, как реально всё решала: что заметила первым, что натолкнуло на мысль, какие тулы использовала и почему конкретный трюк вообще сработал. Не причёсанная версия "тут всё очевидно", а реальный процесс, вместе со всеми шероховатостями.
...
|---|---|
| Веб-безопасность | Сайты, формы логина, API, cookies, WordPress, проблемы с авторизацией. |
| Криптография | Слабые хеши, подписи, генераторы случайных чисел, шифрование. |
| Реверс-инжиниринг | Понять, что делает программа, не имея её исходного кода. |
| Форензика | Файлы, картинки, аудио, метаданные, PCAP, скрытые данные. |
| Пвн или эксплуатация бинарников | Ломаем скомпилированные программы, обычно через баги с памятью. |
...
| `objdump` | Тул, который дизассемблирует бинарник обратно в (более-менее) читаемый ассемблер, чтобы понять, что он делает на уровне процессора. |
